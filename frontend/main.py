...
# ----------------------------------------------------------
# Redireciona quem esqueceu a barra no final
@app.get("/Movimex")
async def redirect_movimex():
    return RedirectResponse(url="/Movimex/")

# Entrega explicitamente a raiz do Movimex
@app.get("/Movimex/")
async def serve_movimex_raiz():
    return FileResponse(os.path.join(MOVIMEX_DIST_DIR, "index.html"))

# Entrega os arquivos estáticos (CSS, JS, Imagens) do Movimex
@app.get("/Movimex/{file_path:path}")
async def serve_movimex_files(file_path: str):
    ...
    # Se o React pedir algo da subpasta que não existe, devolve o HTML do Movimex
    return FileResponse(os.path.join(MOVIMEX_DIST_DIR, "index.html"))
...
if os.path.exists(PWA_DIST_DIR):
    ...
    async def serve_pwa(full_path: str):
        # A regra de segurança do KAD: se for rota da API ou do Movimex que passou direto (não deveria), aborta.
